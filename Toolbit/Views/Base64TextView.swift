...
struct Base64TextView: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            // 页面标题
            PageHeader(
                title: ToolType.base64Text.displayName,
                subtitle: ToolType.base64Text.subtitle,
                icon: ToolType.base64Text.icon,
                color: ToolType.base64Text.color
            )
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                ZStack(alignment: .topLeading) {
                    ...
                    if inputText.isEmpty {
                        Text("输入普通文本进行编码，或输入 Base64 进行解码...")
                            .font(.system(.body, design: .monospaced))
                            .foregroundStyle(.tertiary)
